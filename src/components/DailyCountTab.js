...
const DailyCountTab = () => {
    ...
    useEffect(() => {
        async function fetchData(){
            ...
            setRecordedData([
                {heading:'Confirmed', total:response.data.cases, bgColor:'bg-red-100', textColor:'text-red-600', pColor:'text-red-300', today: response.data.todayCases},
                {heading:'Active', total:response.data.active, bgColor:'bg-blue-100', textColor:'text-blue-600', pColor:'text-blue-300'},
                {heading:'Recovered', total:response.data.recovered, bgColor:'bg-green-100', textColor:'text-green-600', pColor:'text-green-300', today: response.data.todayRecovered},
                {heading:'Deceased', total:response.data.deaths, bgColor:'bg-gray-100', textColor:'text-gray-600', pColor:'text-gray-300', today: response.data.todayDeaths}
            ])
        ...
        <div className="mt-10 md:grid md:grid-cols-4 gap-2 text-center">
            {recordedData.map((record)=>
                <div className={`${record.bgColor} ${record.textColor} mx-auto w-56 md:w-full mt-3 py-3 rounded`}>
                    ...
                    {(record.today) && <div className={`mt-3 ${record.pColor} text-md font-semibold`}>+{record.today.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</div>}
